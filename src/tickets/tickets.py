...
class TicketsCog(commands.Cog):
    ...
    @app_commands.command(name="panel", description="Sends the ticket buttons.")
    @app_commands.guilds(discord.Object(id=guild_id))
    @app_commands.default_permissions(administrator=True)
    async def panel(self, interaction: discord.Interaction):
        view = TicketSystem()
        await interaction.response.send_message("Sent the tickets embed and buttons!", ephemeral=True)
        embed = discord.Embed(title=f'{interaction.guild.name} Tickets',
                              description="Click the corresponding button down below to create a ticket!",
